...
public class EstresaTodo extends Thread {
    ...
    public EstresaTodo(ServidorJuego s) 
    {
        puedeTirar = false;
        auxT1 = 0; auxT2 = 0;
    ...
    public void salir()
    {
        try {
            ...
            Logger.getLogger(EstresaTodo.class.getName()).log(Level.SEVERE, null, ex);
        }
            ...
            try {
                ...
                while (!salir) {
                    ...
                    while (!salir && !juegoTerminado) {
                        ...
                        if(Math.random() < 0.001) //Con un .1% de probabilidad se crea un nuevo cliente
                        {
                            EstresaTodo a = new EstresaTodo(servidor);
                            a.registra();
                            try{servidor.registroExtra();}catch(Exception e){}
                            a.juega();
                        }
                        if(Math.random() < 0.001) //Con un .1% de probabilidad el cliente sale del juego
                        {
                            salir();
                        }
                    }
    ...
    public void registra()
    {
        ...
        try {
            ...
            //Aqui acaba registro
            long spentT2 = System.currentTimeMillis() - start2;
            try{servidor.acumula(spentT2);}catch(Exception e){}
            mcAddress = mca;
            mcPort = mcp;
            tcpAddress = tcpa;
            tcpPort = tcpp;
    ...
    public static void main(String args[]) throws InterruptedException
    {
        ...
        EstresaTodo[] hilos = new EstresaTodo[clientes];
        for (int i = 0; i < clientes; i++) {
            hilos[i] = new EstresaTodo(s.servJuego);
            hilos[i].registra();
        }
